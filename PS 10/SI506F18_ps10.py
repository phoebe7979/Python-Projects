...
## Again, to run the tests, you can run the included Python file SI506F18ps9_tests.py
...
sample_fname = open('sample_diction.json','r')
sample_data = sample_fname.read()
sample_photo_rep = json.loads(sample_data)
sample_fname.close()
...
FLICKR_KEY = "6fb8240b7570a46437b83379e4fe7832"
...
## When you have completed this problem, the tags list in the sample_tags_list variable should basically look like this: ['nature', 'mist', 'mountain']
...
## Write code to open that sample_flickr_response.json file and load the python data from that file into a variable called search_result_diction. Remember to close any file you open once you're done with it!

sam_response_fn = open('sample_flickr_response.json','r')
search_result_diction=json.loads(sam_response_fn.read())
sam_response_fn.close()
...
## Because you will be making requests to the internet in this problem set, to a service that has limits for how many times you can ask for data from them, you will be caching your data, so your code will only make a request to the internet when you don't *already* have the data you need.
...
def get_flickr_data(str, number = 50):
    ...
    params_d["per_page"] = number
    params_d["media"] = "photos"
    unique_ident = params_unique_combination(baseurl, params_d, private_keys=["api_key"])
...
## Referring to the examples from class and from the textbook  about caching, and talking through them carefully, will be VERY helpful here. The work you did in Problem Set 9 may also be a helpful reference, though remember that the functions you wrote in PS7 did NOT cache any data in this manner, and the flickr API requires more query parameters than the OMDB API does!
...
## Read the instructions VERY CAREFULLY to make this problem easier to complete!
...
def get_photo_data_with_caching(id):
...
## [PROBLEM 10]
# Make an invocation to the function get_photo_data_with_caching with the input of the photo id: 45603995912
# Use the data the invocation returns to create a string of the following format and save it to the variable photo_rep_str:
## The photo with id THE PHOTO ID was posted by flickr user FLICKR-USERNAME and has the following tags: LIST OF TAGS
photo_resp = get_photo_data_with_caching(45603995912)


photoid = photo_resp['photo']['id']
print(photoid)
username = photo_resp['photo']['owner']['username']
tagslist = []

for tag in photo_resp['photo']['tags']['tag']:
    tagslist.append(tag['_content'])
tags = ', '.join(tagslist)

photo_rep_str = "The photo with id " + photoid + " was posted by flickr user " + username + " and has the following tags: " + tags
print(photo_rep_str)

## FOR EXAMPLE:
## "The photo with id 45603995912 was posted by flickr user NHPBS and has the following tags: ciaoitalia, event, tuscankitchen"
## Note that the tags from the _content key in the photo dictionary are parsed into a list that is nice to read in the string! You'll have to use the data you get from the function invocation, and put it in the string format.
## For points, DO NOT HARD CODE THE VARIABLE ASSIGNMENT -- USE THE DATA FROM THE FUNCTION INVOCATION!
...
## (We will not offer extra credit for this at this point, but it's a good idea to try it out, or if not, try thinking about it: what would your plan be to do this?)
...
## We have NOT provided tests for this -- you can make your own judgments as to whether or not it works.
