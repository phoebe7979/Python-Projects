...
class Problem6(unittest.TestCase):
    ...
    def test_cache_in_function3(self):
        ...
        testfilestr = testfile.read()
        testfile.close()
        self.assertTrue("per_page-112_tag_mode-all_tags-summer 2013" in testfilestr, "Testing that params and unique identifer setup are correct in get_flickr_data function")
...
class Problem9(unittest.TestCase):
    def test_problem9_invocation(self):
        self.assertTrue(get_photo_data_with_caching("45603995912"))
    def test_problem9_data(self):
        self.assertEqual(sorted(list(get_photo_data_with_caching("45603995912").keys())),sorted(["photo","stat"]) ) # TODO

class Problem10(unittest.TestCase):
    def test_problem10_result(self):
        self.assertEqual(photo_rep_str,"The photo with id 45603995912 was posted by flickr user NHPBS and has the following tags: ciaoitalia, event, tuscankitchen", "testing string generation for problem 10")
